...
async function scriptsFromDirectory(dirpath) {
  ...
  // }
  // console.log("*** EXECUTING browserScript.js:28");
  let scripts = await Promise.all(dir.map(filepath => readFile(filepath, 'utf8')));
  dir.forEach((filepath,index) => {
    const name = path.basename(filepath, '.js');
    result[name] = generateScriptObject(name, filepath, scripts[index]);
  });
  return result;
}
